...
torch.save(calories_model.state_dict(), '../models/calories_model.pth')
joblib.dump(scaler_X, '../models/scalers/calories_scaler_X.pkl')
joblib.dump(label_encoder, '../models/encoders/gender_label_encoder.pkl')
joblib.dump({'mean': y_mean, 'std': y_std}, '../models/scalers/calories_y_scaler_params.pkl')

# Train and Save Nutrient Prediction Models for Each Meal
meals = ['breakfast', 'lunch', 'dinner']
for meal in meals:
    data = pd.read_csv(f"datasets/{meal}_data.csv")
    ...
    torch.save(model.state_dict(), f'../models/{meal}_nutrient_model.pth')
    joblib.dump(scaler_X_meal, f'../models/scalers/{meal}_scaler_X.pkl')
    joblib.dump(scaler_y_meal, f'../models/scalers/{meal}_scaler_y.pkl')
    joblib.dump(targets, f'../models/{meal}_targets.pkl')
    data.to_pickle(f'../models/{meal}_data.pkl')
